...
if ($_POST) {
    ...
    $paie = $_POST['paie'];
    $remise = $_POST['remise'];
    $link_fail = "liste_examen_suite_review.php";
    $link_sucess = "liste_examen_checker.php";
...
?>
    ...
    $cnt = 0;
    $cnt_pers = 0;
    $query  = "SELECT * from regler_examen where id_reg_exa='$id_reg_exa' and id_exa='$id_exa and payer!=0'";
    $q = $conn->query($query);
    ...
    if ($cnt_pers == 1 and $paie != 0) {
        ...
        $somme_total = 0;
        $versement = 0;
        foreach ($id_type_exa as $cle => $data_item) {
            // echo $cle .'=>'. $data_item;
            $query  = "SELECT * from examen_exa where  id_exa='$id_exa' and id_type_exa='$id_type_exa[$cle]'";
            $q = $conn->query($query);
            while ($row = $q->fetch_assoc()) {
            ...
            if ($payer >= 0) {
                ...
                if ($somme_exa - $payer_sum > 0) {
                    ...
                    $query1 = "UPDATE examen_exa SET  etat=:etat
                            WHERE id_exa = '$id_exa' and id_type_exa='$id_type_exa[$cle]' ";
                ...
                $query1 = "UPDATE examen_exa SET  payer=:payer
                            WHERE id_exa = '$id_exa' and id_type_exa='$id_type_exa[$cle]' ";
        ...
        $query1 = "UPDATE regler_examen SET  payer=:payer, remise=:remise, somme=:somme, id_paie=:id_paie, id_caisse=:id_caisse, date_reg_paie=:date_reg_paie
        ...
        $etat = -1;
        $query1 = "UPDATE regler_examen SET  etat=:etat, date_reg_paie=:date_reg_paie
                WHERE  id_exa='$id_exa' and id_reg_exa='$id_reg_exa'";
        ...
        if ($somme_total - $payer_total == 0) {
            ...
            $etat = 1;
            $date_reg_paie = date('Y-m-d');
            $query1 = "UPDATE regler_examen SET  etat=:etat, id_caisse=:id_caisse, date_reg_paie=:date_reg_paie
                WHERE  id_exa='$id_exa' and id_reg_exa='$id_reg_exa'";
        ...
        ?>
            <script>
                ...
                window.location.href = '<?= $link_sucess ?>?witness=1';
            </script>
        <?php
        } else {
        ?>
