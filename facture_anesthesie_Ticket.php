...
$id_reg_anes=$_REQUEST['id_reg_anes'];
...
//    Tes requetes
    ...
    $sql = "SELECT * from regler_anesthesie where id_reg_anes = '$id_reg_anes'";
    ...
    foreach ($tables as $table) {
        $ref_reg_anes = $table['ref_reg_anes'];
...
$pdf->Text(25, 10, 'CLINIQUE CATHOLIQUE SAINTE MARIE');
$pdf->SetFont('Arial','',8);
$pdf->Text(15, 16, 'SOA CENTRE');
$pdf->Text(30, 22, '+237 ');
$pdf->Line(5,24,75,24);
$pdf->SetFont('Arial','',8);
...
$pdf->Text(30, 40, 'CLINIQUE CATHOLIAUE SAINTE MARIE');
...
foreach ($id_medis as $id_med){
    $pdf->SetFont('Arial','',6);
    $pdf->Text(2, 78+$i, strtoupper($type_anes));
    $pdf->Text(45, 78+$i, '1');
    $pdf->Text(53, 78+$i, $somme);
    $pdf->Text(68, 78+$i, $somme);
    $pdf->line(1,80+$i,79,80+$i);
    $i += 4;
}
$pdf->SetFont('Arial','B',8);
