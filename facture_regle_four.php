...
if (!empty($id)) {
    ...
    $query = "SELECT DISTINCT count(id_regle_four) as total from regle_fournisseur where id_regle_four='$id_ch' ";
    $q = $db->query($query);
    while ($row = $q->fetch()) {
    ...
    $query = "SELECT * from regle_fournisseur where id_regle_four=$id_ch";
    $q = $db->query($query);
    while ($row = $q->fetch()) {
        ...
        foreach ($tables as $table) {
            ...
            $ref_four = $table['ref_four'];
            $raison_social_four = $table['raison_social_four'];
            $tel_four = $table['tel_four'];
            $pers_contact_four = $table['pers_contact_four'];
            $tel_contact_four = $table['tel_contact_four'];
            $pays_four = $table['pays_four'];
            $ville_four = $table['ville_four'];
            $email_four = $table['email_four'];
        }
...
$pdf->Cell(130, 5, $raison_social_four, 0, 0);
$pdf->Cell(59, 5, 'FACTURE', 0, 1);//end of line

//set font to arial, regular, 12pt
$pdf->SetFont('Arial', '', 12);

$pdf->Cell(130, 5, $ville_four . ', ' . $pays_four, 0, 0);
$pdf->Cell(25, 5, 'Date', 0, 0);
$pdf->Cell(34, 5, $date, 0, 1);//end of line


$pdf->Cell(130, 5, $email_four, 0, 0);
// $pdf->Cell(59 ,5,'',0,1);//end of line
$pdf->Cell(25, 5, 'Facture #', 0, 0);
$pdf->Cell(34, 5, $total, 0, 1);

$pdf->Cell(130, 5, 'Phone: ' . $tel_four, 0, 0);
//end of line
