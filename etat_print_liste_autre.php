...
if (!empty($ref_exa)) {
    ...
    $query = "SELECT * from autre_exa where ref_autre_exa='$ref_autre'";

    $q = $db->query($query);
    while ($row = $q->fetch()) {
        //   $id_exa+=1;
        $id_medis[$id_autre]=$id_autre;
        $date_autre[$id_autre]= $row['date_autre'];
        $id_autre_service[$id_autre]= $row['id_autre_service'];
        $ref_autre_exa[$id_autre] = $row['ref_autre_exa'];
        $id_patient= $row['id_patient'];
        $id_medecin= $row['id_medecin'];
        $qt_autre_exa[$id_autre] = $row['qte_autre_exa'];
        $etat[$id_autre] = $row['etat'];

        $sql = "SELECT DISTINCT * from autres_services where id_autre_service = '$id_autre_service[$id_autre]'";
        ...
        foreach ($tables as $table) {
            $nom_autre_service[$id_autre] = $table['nom'];
            $prix_autre_service[$id_autre]= $table['prix_autre_service'];
        ...
        if(empty($id_autre_service[$id_autre])){
            $nom_type_exa='N/A';
        }
...
$pdf->Cell(62 ,5,'Autres Services ',1,0);
...
foreach ($id_medis as $id_med){
    ...
    $pdf->Text(27, 70+$i, $nom_autre_service[$id_med]);
    $pdf->Text(88, 70+$i, $prix_autre_service[$id_med] );
    $pdf->Text(130, 70+$i, $qt_autre_exa[$id_med]);
    $total_ht = $prix_autre_service[$id_med]*$qt_autre_exa[$id_med];
    $pdf->Text(143, 70+$i, $date_autre[$id_med]);
