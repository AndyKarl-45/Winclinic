...
if(isset($_POST['id_medi']) and  isset($_POST['quantite']) and isset($_POST['id_num_lot']) ){
...
//if($id_materiel[0]!=0 and abs($quantite[0])!=0){
    if($id!=0){
        ...
        $query = " INSERT INTO demande_medicament (heure_debut,date_debut,date_valide,etat_src,responsable,id_perso) 
                     VALUES (:heure_debut,:date_debut,:date_valide,:etat,:resp,:id_perso)";
        ...
        foreach($id_materiel as $cle => $data_item)
        {
            ...
            if($id_materiel[$cle]!=0){
                ...
                $sql = "SELECT * FROM magasin where id_medi='$id_materiel[$cle]' and id_num_lot='$id_num_lot[$cle]' ";
                $stmt = $db->prepare($sql);
                $stmt->execute();
...
//                    $receveur='S';
                    ...
                    $sql = "INSERT INTO demande_materiel ( id_medi,date_debut_mat,id_ask_medi,quantite,etat_src,responsable,id_perso,id_num_lot)
                                VALUES(:id_materiel,:date_debut,:id_ask_eq,:quantite,:etat,:resp,:id_perso,:id_num_lot)";
                    $req = $db->prepare($sql);
