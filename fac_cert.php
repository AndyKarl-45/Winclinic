...
try{
    ...
    $db = new PDO('mysql:host=localhost;dbname=c2164852c_clinic_syges;charset=utf8', 'c2164852c_clinic_root', '_@H#fb*O{IwR');
...
$pdf->SetFont('CenturyGothic-Bold','',18);
$pdf->Text(60,85,'CERTIFICAT MEDICAL');


$pdf->SetFont('Candara','',12);
$pdf->Text(15,105,iconv('UTF-8', 'windows-1252', 'Je soussignée, Dr '.$nom_medecin.', Médecin en service au Centre Médical le Lys des Portiques (CMLP) certifie'));

$pdf->SetFont('Candara','',12);
if($genre_p =='M'){
    $pdf->Text(15,112,iconv('UTF-8', 'windows-1252','avoir examiné ce jour M. '.$nom_patient.'.'));
}else{
    $pdf->Text(15,112,iconv('UTF-8', 'windows-1252','avoir examiné ce jour Mme '.$nom_patient.'.'));
}


$pdf->SetFont('Candara','',12);
$pdf->Text(15,122,iconv('UTF-8', 'windows-1252', 'Il ressort de ce qui précède que l’intéressé présente un état de santé qui nécessite un arrêt de travail'));

$pdf->SetFont('Candara','',12);
$pdf->Text(15,129,iconv('UTF-8', 'windows-1252', 'de sept ('.$nb_jour.') jour à compter du '.$date_debut.' au '.$date_fin.' inclus, sauf complication.'));

$pdf->SetFont('Candara','',12);
$pdf->Text(15,136,iconv('UTF-8', 'windows-1252', 'En foi de quoi le présent certificat est délivré pour servir et valoir ce que de droit.'));
...
$pdf->SetFont('Candara-Bold','',14);
$pdf->Text(130,155,iconv('UTF-8', 'windows-1252', 'Fait à Douala, le '.$annee));
$pdf->SetFont('Candara-Bold','',14);
$pdf->SetTextColor(0 , 112, 192);
$pdf->Text(100,165,iconv('UTF-8', 'windows-1252', $nom_medecin));
