                        ...
                        <div class="table-responsive">
                            <table class="table table-border table-striped custom-table mb-0" id="dataTable">
                                ...
                                <tbody>
                                    ...
                                    $query = "SELECT  * FROM $service WHERE id_caisse = '$id_caisse' and  payer+remise = somme and etat=1 group by $date_service[$i] order by $date_service[$i] desc";
                                    $db->exec("SET sql_mode = REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', '');");
                                    $q = $db->query($query);
                                    while ($row = $q->fetch()) {
                                        echo   $id_exa = $row['id_exa'];

                                        $sql = "SELECT  Count($id_type_service) as qte, date_exam as date_service, $id_type_service as id_type  FROM examen_exa WHERE  etat=1 and id_exa='$id_exa' group by date_exam order by date_exam desc";
