...
if ($_POST) {
    ...
    $remise = $_POST['remise'];
    $paie = $_POST['paie'];
    $link = 'liste_hospitalisation_checker.php';
    ...
    $cnt = 0;
    $cnt_pers = 0;
    $query  = "SELECT * from regler_hosp where id_reg_hosp='$id_reg_hosp' and id_hosp='$id_hosp' and payer!=0";
    $q = $conn->query($query);
...
?>
        <script>
            ...
            window.location.href = '<?= $link ?>?witness=-4';
        </script>
        <?php
    }
    ...
    if ($cnt_pers == 1  and $paie != 0) {
        if ($cnt == 0) {
            if ($somme - ($payer + $remise) >= 0) {
                ...
                $query1 = "UPDATE regler_hosp SET  payer=:payer, remise=:remise, id_paie=:id_paie, id_caisse=:id_caisse
            ...
            if ($somme - ($payer_sum + $remise_sum) >= 0) {
                ...
                $query1 = "UPDATE regler_hosp SET  payer=:payer, remise=:remise, id_paie=:id_paie
                            WHERE id_hosp = '$id_hosp'and id_reg_hosp='$id_reg_hosp'";
