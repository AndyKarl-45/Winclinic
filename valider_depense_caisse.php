...
if ($_REQUEST) {
    ...
    if ($quantite_final_src >= 0) {
        ...
        $sql = "INSERT INTO historique_caisse (id_caisse,ref_caisse,id_beneficiaire,id_perso,montant_sortie,date_hist,statut,type_beni)
                      VALUES (?,?,?,?,?,?,?,?)";
        $req = $db->prepare($sql);
        $req->execute(array($id_caisse, $ref_caisse, $id_beneficiaire, $id_perso, $somme, $date_hist, $statut, $type_beni));

?>

        <script>
            //alert('Demande n\'a pas été valider.');
            window.location.href = 'liste_depense_caisse.php?witness=1';
        </script>
    <?php
