...
if (!empty($id_caution)) {
  ...
  $sql = "SELECT * from caution where id_caution = '$id_caution'";
  ...
  foreach ($tables as $table) {
    $id_medi = $table['id_caution'];
    $id_medis[$id_medi] = $id_medi;
    $id_caution[$id_medi]  = $table['id_caution'];
    $ref_caution  = $table['ref_caution'];
    $id_patient  = $table['id_patient'];
    $id_perso  = $table['id_perso'];
    $montant[$id_medi]  = $table['montant'];
    $date_modif  = $table['date_modif'];
    $date_caution  = $table['date_caution'];
...
foreach ($id_medis as $id_med) {
  ...
  $pdf->SetFont('Arial', '', 7);
  $pdf->Text(6, $pos1 + $i, strtoupper('Caution'));
  $pdf->Text(52, $pos2 + $i, 1);
  $pdf->Text(58, $pos3 + $i, $montant[$id_med]);

  $total_ht = $montant[$id_med];
...
if ($cout == 13) {
  ...
  $pdf->SetFont('Arial', 'B', 7);
  $pdf->Text(5, 20, iconv('UTF-8', 'windows-1252', 'Total caution'));
  $pdf->Text(67, 20, $somme);
  $pdf->line(5, 21, 77, 21);
  ...
  // Affichage des montants
  $pdf->SetFont('Arial', 'B', 7);
  $pdf->Text(5, 80, iconv('UTF-8', 'windows-1252', 'Total caution'));
