                    ...
                    ?>
                        <b>
                            ...
                            <ul class="nav nav-pills" style="float: right; margin-right: 20px ;">
                                <li class="nav-item">
                                    <a class="nav-link" href="liste_con_suite_review.php">
                            ...
                            <?php if ($lvl != 2) { ?>
                                ...
                                <ul class="nav nav-pills" style="float: right; margin-right: 20px ;">
                                    <li class="nav-item">
                                        <a class="nav-link" href="liste_con_fabrication.php">

                                            <?php
                                            if ($lvl == 5) {
                                                $query = "SELECT count(DISTINCT ref_con) as total from consultation where (id_medecin='$id_perso_session' and etat=2 and open_close!=1) or (id_medecin=0  and etat=2 and open_close!=1)";
                                            } elseif ($lvl == 3) {
                                                $query = "SELECT count(DISTINCT ref_con) as total from consultation where (id_nurse='$id_perso_session' and etat=2 and open_close!=1) or (id_nurse=0 and etat=2 and open_close!=1) ";
                                            } else {

                                                $query = "SELECT count(DISTINCT ref_con) as total from consultation where etat=2 and open_close!=1";
                                            }

                                            // $query = "SELECT count(DISTINCT ref_con) as total from consultation where etat=2 ";

                                            $q = $db->query($query);
                                            while ($row = $q->fetch()) {
                                                echo ' Consultation en cours [' . $row['total'] . ']';
                                            }
                            ...
                            <ul class="nav nav-pills" style="float: right; margin-right: 20px ;">
                                <li class="nav-item">
                                    <a class="nav-link" href="liste_con_parametre.php">

                                        <?php


                                        $query = "SELECT count(DISTINCT ref_con) as total from consultation where etat=1 ";

                                        $q = $db->query($query);
                                        while ($row = $q->fetch()) {
                                            echo ' Paramètres [' . $row['total'] . ']';
                                        }
                    ...
                    <div class="table-responsive">
                        <table class="table table-border table-striped custom-table mb-0" id="dataTable">
                            ...
                            <tbody>
                                <?php
                                if ($lvl == 5) {
                                    $query = "SELECT * from consultation where id_medecin='$id_perso_session' and etat != 1 and etat!= 2 and open_close!=1 ";
                                } elseif ($lvl == 3) {
                                    $query = "SELECT * from consultation where id_nurse='$id_perso_session' and etat != 1 and etat!= 2  and open_close!=1";
                                } else {

                                    $query = "SELECT * from consultation where open_close!=1 and etat != 1 and etat!= 2";
                                }
