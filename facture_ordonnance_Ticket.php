...
try{
    $db = new PDO('mysql:host=localhost;dbname=c2164852c_clinic_syges2;charset=utf8', 'root', '');
...
//    Tes requetes
    ...
    $sql = "SELECT * from regler_ordo where id_reg_ordo = '$id_reg_ordo'";
    ...
    foreach ($tables as $table) {
        $ref_reg_ordo = $table['ref_reg_ordo'];
        $id_patient=$table['id_patient'];
        $payer=$table['payer'];
        $remise=$table['remise'];
        $somme_argent=$table['somme'];
        $ref_ordo = $table['ref_ordo'];
...
//            Get drug's name
        ...
        if(empty($id_type_ordo)){
            $type_ordo='N/A';
        }
        if(empty($ref_ordo)){
            $ref_ordo='N/A';
        }
        if(empty($nom_caissier)){
            $nom_caissier='N/A';
        }
...
//}
$pos0=30;
$max_rows_first_page = 13; // Nombre maximal de lignes de médicaments sur la première page
...
//            $pdf->Cell(10, 30, '', 1, 1);
        ...
        $pdf->SetFont('Arial', '', 7);
        $pdf->Text(6, $pos1 + $i, strtoupper($nom_medi[$id_med]));
...
if($cout == 13 ){
    ...
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->Text(5, 20, iconv('UTF-8', 'windows-1252', 'Montant à payer'));
    $pdf->Text(67, 20, $somme_argent);
    $pdf->line(5, 21, 77, 21);

    $pdf->Text(5, 26, 'Montant payer');
    $pdf->Text(67, 26, $payer);
    $pdf->line(5, 27, 77, 27);

    $pdf->Text(5, 32, iconv('UTF-8', 'windows-1252', 'Reste à payer'));
    $pdf->Text(67, 32, abs($somme_argent - $payer));
    $pdf->line(5, 33, 77, 33);

    $pdf->Text(5, 38, 'Remboursement');
    $pdf->Text(67, 38, abs($somme_argent - ($payer + $remise)));
    $pdf->line(5, 39, 77, 39);
...
// Affichage des montants
    ...
    $pdf->Text(67, 92, abs($somme_argent - $payer));
    $pdf->line(5, 93, 77, 93);

    $pdf->Text(5, 98, 'Remboursement');
    $pdf->Text(67, 98, abs($somme_argent - ($payer + $remise)));
    $pdf->line(5, 99, 77, 99);
