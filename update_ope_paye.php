...
if ($_POST) {
    ...
    $cnt = 0;
    $cnt_pers=0;
    $query = "SELECT * from regler_ope where id_reg_ope='$id_reg_ope' and id_ope='$id_ope'";
    $q = $conn->query($query);
                ...
                if($somme-($payer_init+$remise_init)===0){
                    ?>
                        <script>
                            ...
                            window.location.href = 'liste_operation_checker.php?witness=-4';
                        </script>
                        <?php
                }
            ...
            $query  = "SELECT * from caution where id_patient ='$id_patient' ";
                ...
                if( $paie === $mode_caution ){
                    ...
                    if($montant_caution!=0 ){
                        ...
                        <script>
                            //  alert('client n\'a pas été mis à jour.');
                            window.location.href = 'liste_operation_checker.php?witness=-3';
                        </script>
                        <?php
                    }
    ...
    if ($cnt_pers == 1 and $paie!=0) {
        if ($cnt == 0) {
            if ($somme - ($payer+$remise) >= 0) {
                ...
                $query1 = "UPDATE regler_ope SET  payer=:payer, remise=:remise, id_paie=:id_paie,id_caisse=:id_caisse
                    WHERE id_ope = '$id_ope'and id_reg_ope='$id_reg_ope'";
            ...
            if ($somme - ($payer_sum+$remise_sum) >= 0) {
                ...
                if( $paie === $mode_caution ){
                        ...
                        if($quantite_final_src <= 200000){
                            $date_reg_ope=date('Y-m-d');
                            
                            $query1 = "UPDATE regler_ope SET  payer=:payer, remise=:remise, id_paie=:id_paie,date_reg_ope=:date_reg_ope, id_caisse=:id_caisse
                    WHERE id_ope = '$id_ope'and id_reg_ope='$id_reg_ope'";
                        ...
                        <script>
                            //  alert('client n\'a pas été mis à jour.');
                            window.location.href = 'liste_operation_checker.php?witness=-2';
                        </script>
                        <?php
                        }
                        ...
                        if($somme-($payer_sum+$remise_sum)==0 ){
                                $etats=1;
                                $query1 = "UPDATE operation SET  etat=:etat
                                WHERE id_ope = '$id_ope'";
