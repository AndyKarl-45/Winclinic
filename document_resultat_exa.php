...
if (!empty($ref_exa) and !empty($id_type_exa) ) {
    ...
    $cpt=0;
     ...
     $query = "SELECT * from examen_exa where ref_exam_exa='$ref_exa' and id_type_exa='$id_type_exa'";
        ...
        $id_exa= $row['id_exa'];
          ...
          $date_exam[$id_exa] = $row['date_exam'];
          $id_type_exa[$id_exa] = $row['id_type_exa'];
          $qt_exam_exa[$id_exa] = $row['qte_exam_exa'];
...
$pdf->Text(60, 20, 'RESULTATS DES EXAMENS   ');
...
$pdf->Text(5, 56, 'Laborantin : '.iconv( 'UTF-8' ,'Windows-1252', ucwords(strtolower($nom_lab))));
...
$pdf->Cell(12 ,5,'RANG',1,0);
$pdf->Cell(62 ,5,'TYPE D\'ECHANTILLON ',1,0);
$pdf->Cell(45 ,5,'REMARQUE',1,0);
$pdf->Cell(13 ,5,'RESULTTAT',1,0);
...
foreach ($id_medis as $id_med){
    ...
    $total_ht = $prix_t_exa[$id_med]*$qt_exam_exa[$id_med];
