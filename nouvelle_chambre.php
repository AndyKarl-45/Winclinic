...
$id_cat_exa=6;
$query = "SELECT * from lits where open_close!=1 and nom like 'Pediatric Ward 01%'  ";
    $q = $db->query($query);
    while($row = $q->fetch())
    {
        $id_lit  =$row['id_lit'];
     $query = "UPDATE lits SET id_chambre=:id_chambre WHERE id_lit ='$id_lit'";

    $req = $db->prepare($query);

    // Bind parameters to statement
    $req->bindParam(':id_chambre', $id_cat_exa);
    $req->execute();
}
