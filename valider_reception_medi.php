...
foreach($tables as $table)
{
    ...
    $sql = "SELECT * FROM magasin where id_medi='$id_medi' and id_num_lot LIKE '$id_num_lot' ";
    $stmt = $db->prepare($sql);
    $stmt->execute();
    ...
    if($quantite_reste>=0){
        $query1  = " UPDATE magasin  SET    qt_com=:quantite WHERE id_medi = '$id_medi' and id_num_lot LIKE '$id_num_lot' ";
        ...
        $tot=0;
        $today=date('Y-m-d');
        $sql = "SELECT  * FROM pharmacie where id_medi='$id_medi' and  id_num_lot LIKE '$id_num_lot' and date_fab > '$today'";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        ...
        if ($tot != 0) {
            ...
            $query1 = "UPDATE pharmacie SET quantite=:quantite where id_medi='$id_medi' and id_num_lot LIKE '$id_num_lot'";
...
//            echo $t++.'</br>';
            ...
            $query = " INSERT INTO pharmacie (id_medi,nom_medi,quantite,date_phar,id_num_lot,date_exp,date_fab) 
                     VALUES (:id_medi,:nom_medi,:quantite,:date_phar,:id_num_lot,:date_exp,:date_fab)";
        ...
        if($quantite_reste==0){
            $query = "DELETE FROM magasin WHERE id_medi='$id_medi' and id_num_lot LIKE '$id_num_lot' ";
