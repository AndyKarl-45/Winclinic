...
//            $password_retype = $_POST['password_retype'];
    ...
    if($id_medecin!=0 and $id_chirugien==0 and $id_laboratin==0 and $id_nurse==0 and $id_radiologue==0){
        $lvl = 5;
        $type_spe="M";
    ...
    }elseif($id_medecin==0 and $id_chirugien!=0 and $id_laboratin==0 and $id_nurse==0 and $id_radiologue==0){
        $lvl = 8;
        $type_spe="C";
        if ($totals == 0) {
        ...
        if ($totals == 0) {
            ...
            if (strlen($pseudo) <= 100) {
                ...
                // echo "Pass: ". $password.'<br/>';
                $sql = $conn->query("INSERT INTO users_specialistes (id_specialiste, pseudo, password, lvl, type_spe, email, ip, date)
                                                VALUES($id_radiologue,'$pseudo', '$password', $lvl,'$type_spe', '$email','$ip', SYSDATE())");
