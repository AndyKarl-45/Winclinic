...
//    Tes requetes
    ...
    $sql = "SELECT  Count($id_type_service) as qte, $date_service[$i] as date_service, $id_type_service as id_type   FROM $service WHERE id_caisse = '$id_caisse' and  payer+remise = somme and year($date_service[$i]) = $annee and month($date_service[$i]) = $mois group by $date_service[$i] order by $date_service[$i] desc";
...
$pdf->Text(5, 45, 'Mois en cours : '.$months[$mois]);
