...
$id_reg_autre = $_REQUEST['id_reg_autre'];
...
if (!empty($id_reg_autre)) {
    ...
    $sql = "SELECT * from regler_autre where id_reg_autre = '$id_reg_autre'";
    ...
    foreach ($tables as $table) {
        $ref_reg_autre = $table['ref_reg_autre'];
        $payer = $table['payer'];
        $remise = $table['remise'];
        $somme = $table['somme'];
        ...
        $sql = "SELECT * from autre_exa where ref_autre_exa = '$ref_reg_autre'";
        ...
        foreach ($tables as $table) {
            $id_autre_exa = $table['id_autre_exa'];
            $id_medis[$id_autre_exa] = $id_autre_exa;
            $id_patient[$id_autre_exa] = $table['id_patient'];
            $id_autre_service[$id_autre_exa] = $table['id_autre_service'];
            $amount[$id_autre_exa] = $table['amount'];
            $qte_autre_exa[$id_autre_exa] = $table['qte_autre_exa'];



            $sql = "SELECT DISTINCT * from patient where id_patient = '$id_patient[$id_autre_exa]'";
            ...
            foreach ($tables as $table) {
                $nom_patient[$id_autre_exa] = $table['nom_p'] . ' ' . $table['prenom_p'];
            }

            $sql = "SELECT DISTINCT * from autres_services where id_autre_service = '$id_autre_service[$id_autre_exa]'";
            ...
            foreach ($tables as $table) {
                $autre_service[$id_autre_exa] = $table['nom'];
            }
        }


        if (empty($id_autre_exa)) {
            $autre_service = 'N/A';
        }
        if (empty($ref_reg_autre)) {
            $ref_reg_autre = 'N/A';
        }
...
$pdf->Text(22, $esp_init + 9, $id_reg_autre);
...
$pdf->Text(22, $esp_init + 21, ucfirst(strtolower($nom_patiented)));
...
foreach ($id_medis as $id_med) {
    $pdf->SetFont('Arial', '', 7);
    $pdf->Text(67, $i, strtoupper(iconv('UTF-8', 'windows-1252', $autre_service[$id_med])));
    $pdf->Text(117, $i, '1');
    $pdf->Text(124, $i, $amount[$id_med]);
    $pdf->Text(135, $i, $amount[$id_med]);
